...
// VMImage is the identity the caller fills in, plus the source URL from
// hvperf.yaml.
type VMImage struct {
...
func CreateAndWaitVMImage(ctx context.Context, client dynamic.Interface, image VMImage, timeout time.Duration) error {
	obj, err := render(vmImageTpl, image)
	if err != nil {
		return err
	}
	...
	lw := vmImageListWatch(ctx, client, image.Namespace, image.Name)
	if _, err := client.Resource(VMImageGVR).Namespace(image.Namespace).Create(ctx, obj, metav1.CreateOptions{}); err != nil {
		return fmt.Errorf("create VMImage %s: %w", image.Name, err)
	}

	_, err = watch.UntilWithSync(ctx, lw, &unstructured.Unstructured{}, nil, vmImageCondition(image.Name))
	return err
}
